...
export interface CerbSymbol {
    name: string;
    signature?: string;
    description?: string;
    uri?: string;
    range?: { start: number; end: number }; // character offsets within file
}

export class TokenExtractor {
    private regexSource: string;
    private regex: RegExp;

    constructor(regexSource?: string) {
    ...
    private collectFromString(s: string, set: Set<string>) {
        let m;
...
export function parseCerberusDocSymbols(text: string, uri?: string): CerbSymbol[] {
...
function parseCommandLine(line: string): { name: string; signature?: string; description?: string } | null {
    // possible formats:
